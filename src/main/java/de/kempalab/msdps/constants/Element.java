...
import de.kempalab.msdps.data.PermutationSet;
import de.kempalab.msdps.util.MathUtils;

/**
 * An enumeration of chemical elements, given by their name, atomicNumber and
 * relativeAtomicMass.
 * 
...
public enum Element {
	...
	/**
	 * use a combinatorial approach to determine the masses of all isotopologues of
	 * a molecule E_n, where E is some element and n a natural number. Principles a
	 * based on: Yergey, James A.: A General Approach to Calculating Isotopic
	 * Distributions For Mass Spectrometry. In: International Journal of Mass
	 * Spectrometry 52(1983), 2/3, S. 337-349. - ISSN 1387-3806
	 * 
	 * @param numberOfElements
	 * @param treshFaktor      factor to determine which low abundance masses shall
	 *                         be excluded.
	...
	public MassSpectrum multiElementSpectrum(int numberOfElements, Double treshFaktor) {
		MassSpectrum spectrum = new MassSpectrum(IntensityType.MID, SpectrumType.CENTROIDED);
		IsotopeList isotopes = this.getIsotopes();
		ArrayList<PermutationSet> permutationSets = PermutationSet.allIsotopeCombinations(isotopes.size(),
				numberOfElements);
