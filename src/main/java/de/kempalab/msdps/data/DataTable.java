...
/**
 * A DataTable object can be used to store data that shall be written to a csv file.
 * Data will be added column wise. The size of the columns may differ. When writing to csv shorter columns will be filled
 * by an defined (NA) value.
 * @author sfuerst
 *
 */
...
public class DataTable extends ArrayList<ArrayList<String>> {
	...
	/**
	 * Creates a column with all entries equal to the given value. This method should only be used if the total row
	 * number of this table will not change anymore. Otherwise the created column will miss some entries. 
	 * @param value
	 */
	...
	/**
	 * Creates a column with all entries equal to the given value and inserts it at the given colzmnIndex.
	 * This method should only be used if the total row number of this table will not change anymore.
	 * Otherwise the created column will miss some entries. 
	 * @param value
	 */
	...
	public String toString(String naValue, boolean withHeader) {
		...
		for (int row = 0; row < numberOfRows; row++) {
			for (int column = 0; column < numberOfColumns; column++) {
				...
				try {
					tableStringBuffer.append(String.format("%" + maxCharLength + "s | ", this.get(column).get(row)));
				} catch (IndexOutOfBoundsException e) {
