...
/**
 * If we represent a natural number n as sum of non negative integers n_0,...,n_{k-1} for some natural k < n,
 * then the collection (n_0,...,n_{k-1}) is called a {@link Partition} of n. We will define the length/size of a
 * {@link Partition} (n_0,...,n_{k-1}) as the natural number k in this representation. Two {@link Partition}s are
 * considered to be equal up to permutation if they have the same size and the same set of non zero entries.
 * @author sfuerst
 *
 */
		...
		for (int index = 2; index < this.size(); index++) {
			...
			for (int index2 = 0; index2 < permutationSet.size(); index2++) {
				...
				for (int insertionPoint = 0; insertionPoint < index + 1; insertionPoint++) {
					...
					if (!(permutationSet.contains(newPartition) || temPermutationSet.contains(newPartition))) {
