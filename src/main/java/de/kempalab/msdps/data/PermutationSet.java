...
public class PermutationSet extends ArrayList<Partition>{
	...
	/**
	 * creates a list of {@link PermutationSet}s where one {@link PermutationSet} corresponds to a {@link Partition}
	 * of the numberOfElements and all its permutations. Only {@link Partition}s with length equals to
	 * the numberOfIsotopes are considered. 
	 * @param numberOfIsotopes
	 * @param numberOfElements
	 * @return a list of {@link PermutationSet}s. One set corresponds to a {@link Partition} of the numberOfElements
	 * and all its permutations with length equals to the numberOfIsotopes.
	 */
	public static ArrayList<PermutationSet> allIsotopeCombinations(int numberOfIsotopes, int numberOfElements) {
		...
		if (numberOfIsotopes == 2) {
			for (int summand = numberOfElements; summand >= 0; summand--) {
				PermutationSet permutationSet = new PermutationSet(new Partition(summand, numberOfElements - summand));
				allPermutations.add(permutationSet);
			}
			return allPermutations;
		}
