...
/**
 * Let P represent an isotope pattern consisting of mass peaks p_0,... ,p_n corresponding to masses m_0,... ,m_n (in ascending order),
 * then a MassShift from peak p_i to p_k for i < k is characterized by these peaks and the mass difference m_k - m_i.
 * So we will set:
 * peak1		:= p_i
 * peak_2		:= p_k
 * shiftValue 	:= m_k - m_i
...
public class MassShift {
	...
	/**
	 * Creates a MassShift object from a string, matching the following format:
	 * value[peak1-peak2], e.g 1.0033[1-2]
	...
	public static MassShift fromString(String massShiftString) {
		...
		if (massShiftMatcher.matches()) {
			...
			return new MassShift(Integer.valueOf(peak1), Integer.valueOf(peak2), Double.valueOf(shiftValue));
		} else {
			throw new InputMismatchException(ErrorMessage.INVALID_MASS_SHIFT_PATTERN.getMessage() + "[" + massShiftString + "]");
		}
	}
	
	/**
	 * This shift is said to follow an other shift if this shift starts in the end peak of the other shift.
	 * @param otherShift
