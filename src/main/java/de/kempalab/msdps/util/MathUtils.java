...
public class MathUtils {
	 ...
	 * @return a String representation of the next permutation of the input sequence of bits. The consecutive application of this method
	 * to the string "0011" will produce the following output:
	 * 
	 * 0011
	 * 0101
	 * 0110
	...
	public static String nextPermutation(String binary) {
		/*
		 * The algorithm uses the following principle:
		 * If the last bit in this sequence is on go to the last off bit and switch it with the following on bit.
		 * If the last bit in this sequence is off go to the next to last off bit, switch it with the next on bit and translate all
		 * following on bits to the end of this sequence.
		 */
	...
	/**
	 * This produces a list of all permutations of a binary consisting of n bits, where exactly k ON bits are included.
	 * This can be interpreted as a list of binary representations of all k-elementary subsets on an n-elementary set.
	 ...
	 * e.g permutations(4,2) will return the following list:
	 * 0011
	 * 0101
	 * 0110
	 * 1001
	...
	/**
	 * The method does not check if the set contains duplicates. All elements in the set will be considered as different.
	 ...
	 * @param set
	 * @return a list of subsets from set that are represented by the binarySubsets in the binarySubsetsList.
	 */
	private static ArrayList<ArrayList<Integer>> subsetsFromBinaries(ArrayList<String> binarySubsetsList, ArrayList<Integer> set) {
		ArrayList<ArrayList<Integer>> subsets = new ArrayList<>();
		String bit;
	 ...
	 * @param b
	 * @param allowedError
	 * @return true if the difference between a and b is not bigger than the allowedError, otherwise false.
	 */
	...
	/**
	 * Each {@link Partition} corresponds to a combination of isotopes that creates a molecule consisting of n
	 * elements E (with respect to the isotopes) where n is equals to the numberOfElements. This method creates a list, 
	 * where each entry represents the abundance of the combination of isotopes in the corresponding allCombinations
	 * parameter.
	 * @param allCombinations all possible {@link Partition}s (including their permutations) of the numberOfElements, 
	 * where the length of each {@link Partition} is equal to the size of the {@link IsotopeList}
	 * @param isotopes
	 * @param numberOfElements
	 * @param treshFaktor
	 * @return list of abundances of allCombinatins
	 */
	public static ArrayList<Double> calculateAbundancies(ArrayList<Partition> allCombinations, IsotopeList isotopes,
			Integer numberOfElements, Double treshFaktor) {
...
//		isotopes.sort(new IsotopeAbundancyComparator());
		Double treshhold = treshFaktor * Math.pow(isotopes.get(isotopes.size() - 1).getAbundance(), numberOfElements);
	...
	/**
	 * Calculates the natural abundance of an isotopologue  I using the concept of partitions. If we split the isotopologue into smaller isotopologues
	 * I_0, ... , I_n where each I_k for k in {0,...,n} only consists of isotopes corresponding to the same element E,
	 * then the abundance of I is calculated as the product of the abundances of all I_k.
	 * @param isotopologue
	 * @return
	 */
	public static Double naturalAbundance(IsotopeFormula isotopologue) {
		...
		for (Entry<Element, Integer> element : elementFormula.entrySet()) {
			...
			totalAbundance = totalAbundance * MathUtils.calculateAbundancies(partitions, isotopesForCurrentElemnt, element.getValue(), 0.0).get(0);
		}
