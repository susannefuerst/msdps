...
/**
 * Each peak in an isotope mass pattern can be described by a sequence of MassShifts from where the peak can be reached
 * starting in the p0 peak. This class can be used for such identifications.
 * 
 * @author sfuerst
 *
 */
...
public class MassShiftList extends ArrayList<MassShift> {
	...
	public static final MyLogger LOGGER = MyLogger.getLogger(MassShiftList.class);
	...
	/**
	 * 
	 * @return A list of isotopes that may induce the given mass shifts. These may include Isotope.NONE for a shiftValue of 0.0
	 * or Isotope.UNDEFINED for a none identifiable shiftValue.
	 */
	public IsotopeListList correspondingIsotopes() {
		...
		for (MassShift massShift : this) {
			...
			if (isotopes.equals(new IsotopeList(Isotope.UNDEFINED)) && massShift.getShiftValue() > 2) {
				...
				Double remainingShift = massShift.getShiftValue() - numberOfCarbons * Isotope.C_13.getMassShiftValue();
				if (remainingShift > 0) {
	...
	/**
	 * If this list is given as sequence of mass shifts MS_0, ..., MS_n, then this method returns a connected subsequence
	 * MS_{k_1}, MS_{k_l} of MS, from startPeak to endPeak. If there a more than one possible sequences
	 * the first will be returned if all sequences lead to the same set on isotopes. If they lead to different set
	 * of isotopes a NotUniqueSeqenceException will be thrown.
	 * 
	 * @param startPeak
	 * @param endPeak
	 * @return A connected subsequence from startPeak to endPeak. If there is no connected subsequence, then an empty list will be returned. 
	 */
	 ...
	 * @param startPeak
	 * @param endPeak
	 * @return This list reduced by all mass shifts that start before the startPeak or end after the endPeak.
	 */
	...
	/**
	 * A sequence MS_0,..., MS_n of mass shifts with corresponding peaks peak1_0, peak2_0, ..., peak1_n, peak2_n
	 * is connected from startPeak to endPeak if for all i in {0,...,n-1}: peak2_i = peak1_{i+1} and 
	 * peak1_0 == startPeak, peak2_n == endPeak. That means for two consecutive mass shifts
	 * MS_i and MS_{i+1} the end peak of MS_i is the starting peak of MS_{i+1} and the start peak of MS_0 is given by startPeak
	 * and the end peak of MS_n is given by endPeak. 
	 * 
	 * @param startPeak
	 * @param endPeak
	 * @return true if this sequence of mass shifts starts in startPeak, ends in endPeak and is connected, otherwise false
	 */
	...
	/**
	 * A sequence MS_0,..., MS_n of mass shifts with corresponding peaks peak1_0, peak2_0, ..., peak1_n, peak2_n
	 * is connected if for all i in {0,...,n-1}: peak2_i = peak1_{i+1}. That means for two consecutive mass shifts
	 * MS_i and MS_{i+1} the end peak of MS_i is the starting peak of MS_{i+1}. 
	 *   
...
//				LOGGER.info(this.toString() + " is not connected.");
	...
	/**
	 * @return the string representation of the MassShifts concatenated by "_".
	 * E.g. 0.0[0-0]_1.0033[0-1]_2.0[1-2]_0.9996[2-3]
	 */
	@Override
	...
	/**
	 * Two MassShiftLists may refer to different peak sequences (p_k,p_k+1) and (p'_k,p'_k+1).
	 * If these sequences lead to the same set of isotopes we will regard the two MassShiftLists as equal.
	 * 2.0067[0-1]_1.0033[1-2]_1.0034[2-4]_0.9996[4-5]
	 * 2.0067[0-1]_1.0033[1-2]_0.9996[2-3]_1.0034[3-5]
	 * @return true if the peak sequences of both MassShiftLists lead to the same set of isotopes, otherwise false.
	 */
